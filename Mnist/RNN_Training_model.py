...
def RNN(X, weights, biases):
    ...
    results = tf.matmul(states[1], weights['out']) + biases['out']# 定义数据从cell中变为输出数据时经过的隐藏层
    regularizer(weights['out'])# 将权重weights['out']的正则化项加入到losses集合中
...
with tf.Session() as sess:
    ...
    for step in range(training_iters):
        ...
        sess.run(train_step, feed_dict={x: batch_xs, y: batch_ys, keep_drop: 1})
        ...
        if step % 1000 == 0:
            ...
            print('The current accuracy is ', sess.run(accuracy, feed_dict={x: batch_xs, y: batch_ys, keep_drop: 1}), '.')
            print()
